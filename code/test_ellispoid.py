...
s = [4*1e6,0.3*1e6,-3*1e6] # stress state in Pa
...
def find_location(coefs,point): 
    ...
    n_max = 15 # max number of iteration
    res = 1e-6 # residual
...
toto = find_location(radii, s)
beta = np.zeros_like(toto)
beta[0]= cos(alpha)*toto[0] + sin(alpha)*toto[1]
beta[1] = sin(alpha)*toto[0] + cos(alpha)*toto[2]
beta[2] = toto[2]  
...
ax.plot_surface(x/1e6, y/1e6, z/1e6, rstride=1, cstride=1, cmap="plasma",linewidth=0, antialiased=False)
